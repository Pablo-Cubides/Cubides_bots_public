...
$secretFile = Join-Path $secretsDir 'academic.enc.yaml'
...
Write-Host ''
Write-Host 'Configurarás secretos cifrados para academic.env' -ForegroundColor Cyan
Write-Host '- OPENCLAW_GATEWAY_TOKEN (obligatorio para gateway)' -ForegroundColor Gray
Write-Host '- OPENROUTER_API_KEY (opcional, requerido para ruta A gratis/fallbacks OpenRouter)' -ForegroundColor Gray
Write-Host '- Cualquier otro que quieras (ej: NOTION_API_KEY)' -ForegroundColor Gray
Write-Host ''

$openclawGatewayToken = Read-RequiredSecureValue -Prompt 'Ingresa OPENCLAW_GATEWAY_TOKEN'
$openRouterApiKey = Read-OptionalSecureValue -Prompt 'Ingresa OPENROUTER_API_KEY (opcional, Enter para omitir)'
...
Add-Member -InputObject $envObj -MemberType NoteProperty -Name 'OPENCLAW_GATEWAY_TOKEN' -Value $openclawGatewayToken -Force
...
$extraKeysRaw = Read-Host 'Ingresa nombres de secretos adicionales separados por coma (ej: NOTION_API_KEY,ANOTHER_KEY) o Enter para omitir'
if (-not [string]::IsNullOrWhiteSpace($extraKeysRaw)) {
    ...
    foreach ($key in $extraKeys) {
        if ($key -eq 'OPENCLAW_GATEWAY_TOKEN') {
            continue
        }
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw 'Falló el cifrado con sops al guardar secrets/academic.enc.yaml'
    }
