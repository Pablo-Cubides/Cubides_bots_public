...
if (-not (Test-Path $keyFile)) {
    throw 'No existe .age/keys.txt. Ejecuta primero scripts/secrets-setup-business.ps1'
}
if (-not (Test-Path $secretFile)) {
    throw 'No existe secrets/business.enc.yaml. Ejecuta primero scripts/secrets-setup-business.ps1'
}

$env:SOPS_AGE_KEY_FILE = $keyFile

$keyName = if (-not [string]::IsNullOrWhiteSpace($KeyName)) { $KeyName.Trim() } else { (Read-Host 'Nombre del secreto para Socio (ej: TELEGRAM_BOT_TOKEN)').Trim() }
